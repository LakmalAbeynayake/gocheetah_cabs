...
class Users extends CI_Controller
{
    ...
    function edit()
    {
        ...
        $data['user_group_list'] =  $this->user_group_model->get_user_groups();
    ...
    function profile(){
        ...
        $data['cus_count'] = $this->customers_model->get_cus_count_by_user($user_id);
        $data[''] = $this->customers_model->get_cus_count_by_user($user_id);
    ...
    public function login()
    {
        ...
        $this->form_validation->set_rules('user_password', 'Username', 'required');
        if ($this->form_validation->run() == FALSE) {
            ...
            $user_id       = $this->user_model->login($user_username, $user_password);
            ...
            if ($user_id) {
                ...
                $ss_user_image        = $data['user_details']['user_image'];
                $sesdata              = array(
                ...
                $this->common_model->add_user_activitie("User Login");
            } else {
                $st = array(
                    'success' => false,
                    'validation' => validation_errors()
    ...
    public function save_user()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $last_id = $this->common_model->save($user_data, 'user');
            if ($last_id) {
                $st = array(
                    'success' => true,
                    'validation' => 'Done!',
    ...
    public function update_user()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $num_rows = $this->user_model->update_user($user_data, $user_id);
            if ($num_rows) {
    ...
    function get_list()
    {
        ...
        $values = $this->user_model->get_users($branch_id, $group_id);
        $data = array();
        if (!empty($values)) {
            foreach ($values as $users) {
                ...
                if ($users->user_status == 1) {
                    $row_action .= ' <li><a style="cursor: pointer;" onClick="disableUserData(' . $users->user_id . ')"><i class="fa fa-minus-circle"></i> Disable User</a></li>';
                }
                if ($users->user_status == 0) {
                    $row_action .= ' <li><a style="cursor: pointer;" onClick="enableUserData(' . $users->user_id . ')"><i class="fa fa-check"></i> Enable User</a></li>';
                }
                $row_action .= '<li><a href="' . base_url() . 'users/edit/' . $user_id . '"><i class="fa fa-edit"></i> Edit User</a></li>
								<li class="divider"></li>
                                <li><a href="' . base_url() . 'users/change_pw/' . $user_id . '"><i class="fa fa-pencil"></i> Change Password</a></li>
                            </ul>
                        </div>';
                $row[] = $row_action;
                $data[] = $row;
    ...
    function permissions()
    {
        $this->load->model('warehouse_model');
        //$this->load->model('customers_model');
        $data['main_menu_name'] = 'settings';
        $data['sub_menu_name']  = 'user_permissions';
        ...
        $this->load->view('user/user-permissions', $data);
    }
    function get_list_user_group()
    {
        $values = $this->user_model->get_all_user_group_list();
        $data = array();
        if (!empty($values)) {
            foreach ($values as $users) {
                $row = array();
                ...
                $row[] = $users->user_group_name;
                $actionTxtEnable = '<a href="#" onClick="enable_group(' . $users->user_group_id . ')" class="btn btn-xs btn-success tooltips" data-placement="top" data-original-title="Enable Group"><i class="fa fa-check"></i></a> &nbsp;';
                $actionTxtDisable = '<a href="#" onClick="disable_group(' . $users->user_group_id . ')" class="btn btn-xs btn-warning tooltips" data-placement="top" data-original-title="Disable Group"><i class="fa fa-minus-square"></i></a> &nbsp;';
                $actionTxtUpdate = '<a href="' . base_url('users/edit_permissions') . '/' . $users->user_group_id . '" class="btn btn-xs btn-info tooltips" data-placement="top" data-original-title="Edit Group Permissions"><i class=" fa fa-tasks"></i></a> &nbsp;';
                $actionED = $users->user_group_status ? $actionTxtDisable : $actionTxtEnable;
                $row[] = '<div style="display:flex;justify-content: center;">' . $actionTxtUpdate . $actionED . '</div>';
                $data[] = $row;
            }
            $output = array('data' => $data);
            echo json_encode($output);
    ...
    {
        /* $data['main_menu_name']  = 'users';
        $data['sub_menu_name']   = 'list-user';
        $data['title']           = 'users permissions';
        $user_group_id = $this->uri->segment('3');
        $data['user_group_id'] = $user_group_id;
        $data['perm_and_page_list'] = $this->common_model->list_nav_menu();
        $data['user_group_group_info'] = $this->user_model->get_user_group_info_by_id($user_group_id);
        $this->load->view('user/user-permissions-edit', $data); */
        $data['main_menu_name']  = 'users';
        $data['sub_menu_name']   = 'list-user';
        $data['title']           = 'users permissions';
        $user_group_id = $this->uri->segment('3');
        $data['user_group_id'] = $user_group_id;
        $data['perm_and_page_list'] = $this->common_model->get_all_navs_no_filter();
        //echo "<pre>";
        //print_r($data['user_group_permission_page_list'] );exit;
        $data['list_perms'] = $this->common_model->list_perms();
        $data['user_group_group_info'] = $this->user_model->get_user_group_info_by_id($user_group_id);
        $this->load->view('user/user-permissions-edit-v2', $data);
    }
    
    function save_user_group_permissions()
    {
        ...
        $data['perm_and_page_list'] = $this->common_model->list_menu();
        $this->common_model->reset_group_permission($user_group_id);
        $status     = 1;
        $validation = "Done!";
        /* TRANSACTION START */
        $this->db->trans_begin();
        $this->db->trans_strict();
        foreach ($data['perm_and_page_list'] as $row) {
            $rowname_view   = 'row_view_' . $row['menu_id'];
            if (!$this->input->get($rowname_view))
                continue;
            $where          = array(
                'menu_id' => $row['menu_id'],
            ...
            $has_permission = ($this->input->get($rowname_view) == 1) ? 1 : 0;
            $perm_data      = array(
                "has_permission" => $has_permission
            );
            $result         = $this->common_model->update_rule($where, $perm_data);
            if ($result == 0) {
                $rule = array(
                    'menu_id' => $row['menu_id'],
                    'group_id' => $user_group_id,
                ...
                $result = $this->common_model->add_rule($rule);
                /* add missing rule */
                if ($result == 0) {
                    /* unable to add rule */
                    $status     = 0;
                    ...
                    $this->user_group_model->clear_table();
                }
            }
        }
        if ($status == 1) {
    ...
    /* DEALERS */
    function srh_dealers(){
        $srh = $this->input->post('search_string');
        $result = $this->user_model->get_dealers($srh);
