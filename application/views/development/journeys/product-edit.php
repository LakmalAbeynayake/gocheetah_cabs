...
<body>
						...
						<div class="col-md-12">
							...
							<div class="panel">
								<div class="panel-heading">
									<h2 class="panel-title"><strong>EDIT PRODUCT</strong></h2>
									<div class="right">
										<a href="<?php echo base_url() ?>products">Products</a> / Edit Product
										<button type="button"><i class="lnr lnr-chevron-left" onclick="javascript:history.back()"></i></button>
										<!-- <button type="button" class="btn-toggle-collapse"><i class="lnr lnr-chevron-up"></i></button> -->
										<!-- <button type="button" class="btn-remove"><i class="lnr lnr-cross"></i></button> -->
									</div>
								</div>
								<form action="" id="add_product_form">
									<input type="hidden" id="product_id" name="product_id" value="<?php echo (isset($product->product_id)) ? $product->product_id : ""; ?>" />
									<div class="panel-body">
										<div class="form-group">
											<label class="col-sm-2 control-label" for="product_name"> Product Name
												*</label>
											<div class="col-sm-9">
												<input type="text" id="product_name" class="form-control" name="product_name" value="<?php echo (isset($product->product_name)) ? $product->product_name : ""; ?>">
											</div>
										</div>
											...
											<div class="col-sm-9">
												<select class="form-control search-select" id="category" name="category">
													...
													<?php foreach ($main_category as $key => $category) { ?>
														<option value="<?php echo $category->cat_id; ?>" <?php echo (isset($product->cat_id)) ? (($category->cat_id == $product->cat_id) ? " selected" : "") : ""; ?>><?php echo $category->cat_name; ?></option>
													<?php } ?>
												</select>
											</div>
										</div>
										<div class="form-group">
											...
											<div id="subcat_data" class="col-sm-9">
												<select data-placeholder="Select category to load sub-categories" id="subcategory" class="form-control search-select" name="subcategory">
													<option selected="selected" value="<?php echo (isset($product->sub_cat_id)) ? $product->sub_cat_name : ""; ?>"></option>
												</select>
											</div>
										</div>
										<div class="form-group">
											<label class="col-sm-2 control-label" for="unit"> Product Unit * </label>
											<div class="col-sm-9">
										...
										<div class="form-group">
											<label class="col-sm-2 control-label" for="si_prefix"> SI Prefix</label>
											<div class="col-sm-9">
												<select class="form-control search-select" id="si_prefix" name="si_prefix">
													<?php
													foreach ($prefix_list as $key => $prefix) {
														if ($prefix->id == $product->product_prefix) {
															if ($prefix->id == 4) {
																echo "<option selected value='$prefix->id'>-No prefix-</option>";
															} else
																echo "<option selected value='$prefix->id'>" . $prefix->si_prefix . "(" . $prefix->si_name . ")</option>";
														} else {
															if ($prefix->id == 4) {
																echo "<option value='$prefix->id'>-No prefix-</option>";
															} else
																echo "<option value='$prefix->id'>" . $prefix->si_prefix . "(" . $prefix->si_name . ")</option>";
														}
													} ?>
												</select>
											</div>
										...
										<div class="form-group collapse">
											...
											<div class="col-sm-9">
												<input type="text" id="product_price" name="product_price" class="form-control auto" data-a-sign="Rs. " data-d-group="2" value="<?php echo (isset($product->product_price)) ? $product->product_price : ""; ?>">
											</div>
										</div>
										...
										<div class="form-group collapse">
											...
											<div class="col-sm-9">
												<input type="text" name="product_cost" id="product_cost" class="form-control auto" data-a-sign="Rs. " data-d-group="2" value="<?php echo (isset($product->product_cost)) ? $product->product_cost : ""; ?>">
											</div>
										</div>
										<div class="form-group">
										...
										<div class="form-group">
											<div class="col-sm-12">
												<p></p>
												<button class="btn btn-primary btn-squared pull-right" type="submit"> Save Changes</button>
											</div>
										</div>
								</form>
							</div>
	...
	<script>
		...
		function add_product(form) {
			if (!$('#product_id').val() > 0) {
				bootbox.alert("ERROR. Please contact system administrator.");
				return false;
			}
			$.ajax({
				...
				dataType: "JSON",
				contentType: false, // The content type used when sending data to the server. - application/json
				cache: false, // To unable request pages to be cached
				processData: false, // To send DOMDocument or non processed data file it is set to false
				success: function(data) // A function to be called if request succeeds
				{
					if (data.success) {
						bootbox.alert('<i class="fa fa-check-circle"></i> Product updated successfully');
						setTimeout(() => {
